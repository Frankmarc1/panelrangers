...
import { validateRuc } from './components/utils/validateRuc';
import { cloneStore } from './components/helpers/cloneStore';
...
export const CloneStore = ({ id,isVisible, onClose, children }: Props) => {
  ...
  const {idComercio}=router2.query;
  ...
  const handleClone = async () => {
    ...
    const arrayPromises = selectedCompanies.map(async (id) => {
      const pathToStore = `empresas/${idBusiness}/comercios/${idComercio}`;
    ...
    toast.success('Se clono el comercio correctamente');
  };
  ...
  return (
    ...
    <div
      ...
      onClick={handleClose}
    >
      <div className='card bg-white w-2/5 rounded-md h-[40%] shadow-sm border mt-4'>
        <div className='card-header boder-b border-b border-slate-100 mx-2 my-2 flex w-[100%]'>
          <h5 className='flex w-[88%] ml-7 card-title text-medium'>Clonar Empresa</h5>
          <button className='flex justify-end mt-[-1rem] btn bg-slate-100 text-black border-slate-200 hover:bg-slate-100 hover:border-slate-100'
            onClick={() => onClose()}
          >
